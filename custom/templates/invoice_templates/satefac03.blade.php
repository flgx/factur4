...
$detalle[$i][1] = number_format($item->quantity,2);  // Cantidad
...
$detalle[$i][6] = number_format(($item->quantity*$item->price)+($tiva[$idx]*$item->quantity*$item->price)/100,2); // Iva
$impiva[$idx] += $item->quantity*$item->price;
$totiva[$idx] += ($tiva[$idx]*$item->quantity*$item->price)/100;
$totconiva += ($item->quantity*$item->price)+($tiva[$idx]*$item->quantity*$item->price)/100;
$totsiniva += $item->quantity*$item->price;
$i++;
}
...
<table width="89%" border="0" cellpadding="0" cellspacing="0" style="padding-left:5px;">
  <tr>
  ...
  <tr>
    ...
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td align="right" valign="bottom"><span class="titazul"><strong>Factura</strong></span><br>
    <?=$nrofac?>
    </td>
  </tr>
  <tr>
  ...
  <tr>
    <td colspan="5"  >
    <span class="titazul" style="text-transform:uppercase"><strong>{{ $invoice->user->company }}</strong></span><br>
    <span style="color:#CCC">{!! $invoice->user->formatted_address !!}
    ...
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td colspan="3" align="right" valign="top">{{ $invoice->client->name }}<br>
       <span style="color:#CCC">@if ($invoice->client->address) {!! $invoice->client->formatted_address !!}@endif
       @if ($invoice->client->phone) {{ $invoice->client->phone }}<br>@endif
    @if ($invoice->client->email) <a
        href="mailto:{{ $invoice->client->email }}">{{ $invoice->client->email }}</a>@endif
       </span>
    </td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <table width="100%" height="33" border="2" cellpadding="0" cellspacing="0" id="cabdet">
      <tr>
        ...
        <td width="7%"  align="center" valign="middle">Tipus<br> IVA</td>
        <td width="12%"  align="center" valign="middle">Import<br> sin IVA </td>
        <td width="9%"  align="center" valign="middle">IVA</td>
        <td width="12%"  align="right" valign="middle">Import<br> amb IVA</td>
      </tr>
    </table>
    </td>
  </tr>
...
<table width="99%" border="0" cellpadding="0" cellspacing="0" style="padding-left:5px">
  <tr>
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        ...
        <td width="13%" align="right" valign="middle" style="font-size:12px"><?=number_format($impiva[0], 2)?>&nbsp;<?=$tipmod?>&nbsp;</td>
      ...
      <tr>
        ...
        <td align="right" valign="middle" style="font-size:12px"><?=number_format($impiva[1])?>&nbsp;<?=$tipmod?>&nbsp;</td>
      ...
      <tr>
        ...
        <td align="right" valign="middle" style="font-size:12px"><?=number_format($impiva[2])?>&nbsp;<?=$tipmod?>&nbsp;</td>
        ...
        <td bgcolor="#428798" style="color:#FFF"><strong>Total Factura sense IVA</strong></td>
      ...
      <tr>
        ...
        <td bgcolor="#428798" style="color:#FFF"><strong>Total Factura amb IVA</strong></td>
...
<div class="footer">
	<span style="font-size:10px; color:#CCC;">De conformitat amb el disposat per la Llei Orgànica 15/1999, de 13 de desembre, de Protecció de Dades de caràcter personal, dono permís per a que les meves dades siguin incorporades en un fitxer responsabilitat de SATE SERVEIS ASISSTENCIALS i que sigui tractades amb la finalitat de mantenir, desenvolupar i controlar la relació contractual. Així mateix, declaro haver estat informat sobre la possiblitat d'erxercir els drets d'accés, rectificació, cancel·lació i oposició, dirigint-me a SATE SERVEIS ASISSTENCIALS al carrer Sant Joan 8 Baix de roquetes; o bé remetent un missatge de correu electrònic a administracio@sateserveisassistencials.com
    </span>
