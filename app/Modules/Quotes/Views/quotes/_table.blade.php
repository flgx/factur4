<table class="table table-hover" style="height: 100%;">
    ...
    <tr>
        ...
        <th class="hidden-xs">{!! Sortable::link('created_at', trans('fi.date'), 'quotes') !!}</th>
        <th class="hidden-sm hidden-xs">{!! Sortable::link('expires_at', trans('fi.expires'), 'quotes') !!}</th>
        <th>{!! Sortable::link('clients.name', trans('fi.client_name'), 'quotes') !!}</th>
    ...
    @foreach ($quotes as $quote)
        <tr>
            ...
            <td class="hidden-xs">{{ $quote->formatted_created_at }}</td>
